...
if [ ! -f $ISO_PATH ]; then
    echo "\n* Downloading RancherOS ${RANCHEROS_VERSION} to ${ISO_PATH}..."
...
docker-machine create -d virtualbox --virtualbox-boot2docker-url ${ISO_PATH} ${DM_VM_NAME}
...
vboxmanage modifyvm ${TMP_VM_NAME} --memory 2048 --vram 8 --acpi on --ioapic on --cpus 2
